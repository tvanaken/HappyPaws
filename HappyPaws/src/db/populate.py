...
def create_fake_reminders(session):
    med_reminder = Reminder(
        user_id=1, type="Medication", start="2023-12-10T07:00", end="2023-12-10T08:30"
    )
    play_reminder = Reminder(
        user_id=1, type="play", start="2023-12-05", end="2023-12-05"
    )
    exercise_reminder = Reminder(
        user_id=2, type="exercise", start="2023-12-21", end="2023-12-23"
    )
...
def create_fake_breeds(session):
    ...
    session.add(husky)
    american_eskimo = Breed(
        name="American Eskimo", suggested_supplements=None, suggested_exercise="running"
    )
    session.add(american_eskimo)
    bulldog = Breed(
        name="Bulldog", suggested_supplements=None, suggested_exercise="swimming"
    )
    session.add(bulldog)
    session.commit()
